...
对长视频、高字幕量或 evidence-heavy case，建议额外保留：
...
- `routing.carrier_family`
- `routing.support_profile`
- `routing.recall_budget`
- `length_budget.transcript_txt_chars`
- `length_budget.min_note_chars`
- `length_budget.soft_note_char_range`
- `length_budget.compression_ratio`
- `artifacts.section_alignment`
- `artifacts.visual_obligation_ledger`
- `artifacts.review_dir`
- `artifacts.revision_actions`
...
- `routing.*` 应记录最终三轴路由，而不是只记录 runtime prior
- `length_budget.*` 应记录本次计算出的字数 guardrail
...
对于需要复盘 coverage 或选图决策的 case，建议同时保留 `review/`、`work/section_alignment.json` 与 `work/visual_obligation_ledger.json`。
